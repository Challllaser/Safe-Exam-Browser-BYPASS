...
namespace SafeExamBrowser.Monitoring.Display
{
	public class DisplayMonitor : IDisplayMonitor
	{
		...
		public ValidationResult ValidateConfiguration(DisplaySettings settings)
		{
			...
			{
				result.IsAllowed = settings.IgnoreError || UseDebugDisplayErrorBypass();
		...
		private bool UseDebugDisplayErrorBypass()
		{
#if DEBUG
			var enabled = Environment.GetEnvironmentVariable("SEB_DEBUG_IGNORE_DISPLAY_ERRORS") != "0";

			if (enabled)
			{
				logger.Info("Using DEBUG display validation bypass because SEB_DEBUG_IGNORE_DISPLAY_ERRORS=1.");
			}
