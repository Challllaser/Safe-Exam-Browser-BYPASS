	...
	public class WindowGuard : IWindowGuard
	{
		...
		private void Guard(Window window)
		{
			window.ExecuteWithAccess(() =>
			{
				...
				try
				{
					if (AllowDebugScreenCapture())
					{
						logger.Info($"Skipping capture guard for window '{info}' because SEB_DEBUG_ALLOW_SCREEN_CAPTURE is enabled.");
						return;
					}
		...
		private bool AllowDebugScreenCapture()
		{
#if DEBUG
			return Environment.GetEnvironmentVariable("SEB_DEBUG_ALLOW_SCREEN_CAPTURE") != "0";
#else
			return false;
#endif
		}
